...
Issue #8 では、選択ページまたはページ範囲を別PDFへ抽出する非破壊 export に加え、文書全体を明示的な範囲または最大ページ数で複数PDFへ分割する機能も追加しています。
...
- 既存同名ファイルの上書きは既定off。offでは1件でも衝突があればpreflightで全体を拒否し、onでは各targetの `TargetSnapshot` を実行開始前と置換直前に検証する
...
- キャンセルはoutput間で受け付け、現在のatomic exportを中断せず、残りをcancelledとしてreportする
