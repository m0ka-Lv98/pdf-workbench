...
- page extraction は current document を変更しない export 操作として扱い、`CommandHistory` へ積まない。UI は `PageExtractionPlan` を生成して `PdfPageExportService` へ渡し、成功/失敗しても selection、current page、dirty state を変えない
...
現在の文書から別PDFを作る非破壊exportを担当する。Issue #8 Phase 1では selected pages と page range extraction だけを扱い、split / merge / image-to-PDF は別Phaseに残す。
