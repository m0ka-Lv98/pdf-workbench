...
永続ファイルの`source_path`と、アプリ内部で扱う`working_copy_path`、セッション単位の`workspace_directory`を明確に分離する。
...
PDFを開くたびに、`platformdirs`が返すユーザーキャッシュ配下の`pdf-workbench/sessions/<session-id>/working.pdf`へ作業コピーを作成する。`PdfView`と今後の編集処理はこの作業コピーを参照し、タブクローズまたは正常終了時にセッションディレクトリを削除する。
...
Issue #6のPhase 1では、作業コピーと保存用一時PDFもこの配下を利用する。クラッシュ復旧メタデータと外部変更検知は後続PRで追加する。
