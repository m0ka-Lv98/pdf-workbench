...
- transparency は白背景へflatten、黒背景へflatten、または soft mask として保持から選択する。CMYK は ICC profile がある場合だけ sRGB へ変換し、profile がない場合は拒否する
...
- worker thread で逐次処理し、画像全体や全ページを同時に保持しない。cancel は安全な中断点で反映し、既存targetと入力画像を維持する
